...
namespace AuthenticationApi.Infrastructure.Repositories
{
    public class UserRepository(AuthenticationDbContext context, IConfiguration configuration) : IUser
    {
        ...
        public async Task<Response> Login(LoginDTO loginDTO)
        {
            ...
            bool verifiedPassword = BCrypt.Net.BCrypt.Verify(loginDTO.Password, getUser.Password);
            if (!verifiedPassword)
            {
                return new Response(false, $"Invalid credentials");
            }
        ...
        public async Task<Response> Register(AppUserDTO appUserDTO)
        {
            ...
            return result.Entity.Id >0 ? new Response(true, "User registered successfuly"):
                new Response(false, "Invalid data providaded");
        }

        private string GenerateToken(AppUser user) 
        {
            var valueKey = configuration.GetSection("Authentication:Key").Value;
